...
Ms Brown, thanks to the notification, will know that someones has sent something via SecureDrop and connects to the *Journalist Interface* at http://zn3gdzvx2j3rwakh3nh9hw51iqsl7tvqxujnsfmifpflah7ezr2xhwid.onion using the *Journalist Workstation* (another Tails on usb pendrive). In this tutorial you will use the *Admin Workstation* for semplicity. You can find more information about the *Journalist Workstation* :doc:`here <onboarding>`.
...
.. note:: If you try to connect to the *Journalist Interface* with a regular Tor Browser outside of the *Admin Workstation* or the *Journalist Workstation* you will be asked to provide "a private key for this onion service" that you can obtain with ``cat ~/Persistent/securedrop/install_files/ansible-base/app-journalist.auth_private |rev | cut -d: -f 1 | rev`` in the *Admin Workstation*. **In a production environment this should never be done.**
